import { FIRST_SETS, FOLLOW_SETS } from "@/data/compiler-data";

function SetTable({ title, subtitle, data, colorClass }: {
  title: string;
  subtitle: string;
  data: { nonTerminal: string; set: string }[];
  colorClass: string;
}) {
  return (
    <section>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-muted border-b border-border">
              <th className="text-left px-4 py-3 font-semibold text-foreground w-40">Non-Terminal</th>
              ...
              <tr key={row.nonTerminal} className="border-b border-border last:border-0 hover:bg-muted/40 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-1.5">
                    {row.set.split(",").map((s) => s.trim()).map((sym) => (
                      <span
                        key={sym}
                        className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-mono font-medium border ${colorClass}`}
                      >
                        {sym}
                      </span>
                    ))}
