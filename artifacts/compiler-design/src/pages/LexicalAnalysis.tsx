...
import { TOKENS, SYM_KEYWORDS, SYM_IDENTIFIERS, SYM_LITERALS, SOURCE_CODE } from "@/data/compiler-data";
  ...
  return (
    <div className="space-y-8">
      <section>
        ...
        <p className="text-sm text-muted-foreground mb-3">Input source code for the hypothetical language</p>
      ...
      <section>
        ...
        <p className="text-sm text-muted-foreground mb-3">
          {TOKENS.length} tokens identified by the lexical analyzer
        </p>
                    ...
                    <td className="px-4 py-2.5">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium border ${
                          TOKEN_COLORS[token.type] ?? "bg-gray-100 text-gray-700 border-gray-200"
                        }`}
                      >
                        {token.type}
                      </span>
      ...
      <section>
          ...
          {(["keywords", "identifiers", "literals"] as const).map((tab) => (
            ...
            >
              ...
              <span className="ml-2 text-xs opacity-70">
                ({tab === "keywords" ? SYM_KEYWORDS.length : tab === "identifiers" ? SYM_IDENTIFIERS.length : SYM_LITERALS.length})
              </span>
            </button>
          ))}
        </div>
        ...
        <div className="rounded-lg border border-border overflow-hidden shadow-sm">
          {activeSymTab === "keywords" && (
            <SymTable items={SYM_KEYWORDS} label="Keyword" color="bg-blue-100 text-blue-800 border-blue-200" />
          )}
          {activeSymTab === "identifiers" && (
            <SymTable items={SYM_IDENTIFIERS} label="Identifier" color="bg-green-100 text-green-800 border-green-200" />
          )}
          {activeSymTab === "literals" && (
            <SymTable items={SYM_LITERALS} label="Literal" color="bg-amber-100 text-amber-800 border-amber-200" />
          )}
        </div>
      </section>
    </div>
